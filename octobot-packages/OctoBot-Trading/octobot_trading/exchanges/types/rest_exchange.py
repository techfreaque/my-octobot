...
from ccxt.async_support import NotSupported
...
class RestExchange(abstract_exchange.AbstractExchange):
    ...
    async def create_order(self, order_type: enums.TraderOrderType, symbol: str, quantity: decimal.Decimal,
                           ...
                           reduce_only: bool = False, tag: str = None, params: dict = None) -> typing.Optional[dict]:
        async with self._order_operation(order_type, symbol, quantity, price, stop_price):
            created_order = await self._create_order_with_retry(
                ...
                side=side, current_price=current_price, stop_price=stop_price, 
                tag=tag, reduce_only=reduce_only, params=params)
    ...
    async def edit_order_by_replacing(
        ...
        replaced_order = await self.create_order(
            ...
            side=side,
            current_price=order_to_cancel.created_last_price,
            reduce_only=order_to_cancel.reduce_only,
            tag=order_to_cancel.tag,
            params=params,
        )
        if replaced_order:
    ...
    async def _create_order_with_retry(self, order_type, symbol, quantity: decimal.Decimal,
                                       ...
                                       current_price: decimal.Decimal, stop_price: decimal.Decimal, 
                                       reduce_only: bool, tag: str, params) -> dict:
        try:
    ...
    async def _create_specific_order(self, order_type, symbol, quantity: decimal.Decimal, price: decimal.Decimal = None,
                                     side: enums.TradeOrderSide = None, current_price: decimal.Decimal = None, 
                                     stop_price: decimal.Decimal = None,
                                     reduce_only: bool = False, tag: str = None, params=None) -> dict:
        created_order = None
        float_quantity = float(quantity)
        float_price = float(price)
    ...
    async def _create_market_buy_order(
        self, symbol, quantity, price=None, reduce_only: bool = False, tag: str = None, params=None
        ) -> dict:
    ...
    async def _create_market_stop_loss_order(
        self, symbol, quantity, price, side, current_price, tag: str = None, params=None
        ) -> dict:
    ...
    async def _create_limit_stop_loss_order(
        self, symbol, quantity, price, stop_price, side, tag: str = None, params=None) -> dict:
    ...
    async def _create_market_take_profit_order(
        self, symbol, quantity, price=None, side=None, tag: str = None, params=None) -> dict:
