...
BINANCE_ORDER_TAG = "clientOrderId"
...
class BinanceUsdM(exchanges.RestExchange):
    ...
    async def _create_limit_buy_order(
        ...
        params = self.get_cleaned_tag(tag, params)
        if reduce_only:
    ...
    def get_cleaned_tag(self, tag: str, params: dict = None) -> dict:
        params = params or {}
        if tag:
            params[BINANCE_ORDER_TAG] = tag[:36]
        return params
...
class BinanceUsdMAdapter(exchanges.CCXTAdapter):
    ...
    def fix_order(self, raw, **kwargs):
        ...
        fixed[trading_enums.ExchangeConstantsOrderColumns.TAG.value] = order_info.get(
            BINANCE_ORDER_TAG
        )
        return fixed
