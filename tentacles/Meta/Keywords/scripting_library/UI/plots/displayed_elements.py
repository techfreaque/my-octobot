...
class DisplayedElements(display.DisplayTranslator):
        ...
        async with databases.MetaDatabase.database(database_manager) as meta_db:
            ...
            if trading_mode.is_backtestable():
                exchange_name, symbol, time_frame = \
                    await self._adapt_inputs_for_backtesting_results(
                        meta_db, metadata, exchange_name, symbol, time_frame)
            ...
            first_candle_time, last_candle_time = \
                await self._add_candles(graphs_by_parts, candles, exchange_name,
                                        exchange_manager, symbol, time_frame,
    ...
    async def _adapt_inputs_for_backtesting_results(
        ...
        if symbol not in metadata.get(commons_enums.BacktestingMetadata.SYMBOLS.value, []):
            raise commons_errors.MissingExchangeDataError(
    ...
    async def _add_candles(self, graphs_by_parts, candles_list, exchange_name, exchange_manager, symbol, time_frame,
                           run_start_time, run_end_time):
    ...
    async def _get_candles_to_display(self, candles_metadata, exchange_name, exchange_manager, symbol, time_frame,
                                      run_start_time, run_end_time):
